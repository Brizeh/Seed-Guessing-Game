...
class SixNamesGame extends StatefulWidget {
  const SixNamesGame({super.key});

  @override
  State<SixNamesGame> createState() => _SixNamesGameState();
}

class _SixNamesGameState extends State<SixNamesGame> {
  ...
  void nextRound() {
    ...
    // Pick 5 wrong seed names
    final wrongSeeds = List.from(seeds)..remove(correctSeed)..shuffle();
    final wrongNames =
        wrongSeeds.take(5).map((s) => s.name.trim()).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('6 Names → 1 Picture')),
      body: Stack(
        children: [
          Column(
            children: [
